...
from heatbath import heatbath_3dLGT
...
K_array = np.concatenate(
         (np.arange(0.02, 5.0, 0.02)[::-1],
          np.arange(0.01, 5.0, 0.02)))
...
for K in K_array:
    T = 1./K
    ## initialize lattice: half ordered, half disordered
    #lt = np.zeros(size) # all arguments are 0
    #lt1 = np.zeros([size[0]*2, size[1]*2, size[2]*2, 3])
