    ...
    for i_cycle in xrange(Ncycles):
        ...
        for i, j, k in itertools.product(*map(xrange, size[:-1])):
            ...
            for i_r, r in enumerate(r_array):
                ...
                for p in xrange(r):
                    arg += lt1[i+p, j, k, 0]
                    arg += lt1[i+r-1, j+p, k, 1]
                    arg -= lt1[i+p, j+r-1, k, 0]
                    arg -= lt1[i, j+p, k, 1]
                W[i_r] += Re[arg % N]
                arg = 0
                for p in xrange(r):
                    arg += lt1[i, j+p, k, 1]
                    arg += lt1[i, j+r-1, k+p, 2]
                    arg -= lt1[i, j+p, k+r-1, 1]
                    arg -= lt1[i, j, k+p, 2]
                W[i_r] += Re[arg % N]
                arg = 0
                for p in xrange(r):
                    arg += lt1[i, j, k+p, 2]
                    arg += lt1[i+p, j, k+r-1, 0]
                    arg -= lt1[i+r-1, j, k+p, 2]
                    arg -= lt1[i+p, j, k, 0]
                W[i_r] += Re[arg % N]
